...
def test_default_config_toml_documents_core_options() -> None:
    ...
    assert "drop_params = []" in text


def test_drop_params_defaults_to_no_rules() -> None:
    assert config.drop_params_from_config({}) == ()


def test_drop_params_loads_and_normalizes_toml(tmp_path: Path) -> None:
...
""",
    ...
    assert config.drop_params_from_config(loaded) == ("temperature", "top_p")
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            TypeError,
            "compat.drop_params must be an array of strings",
        ),
        (
            '[compat]\ndrop_params = ["temperature", 1]\n',
            TypeError,
            "compat.drop_params must contain only non-empty strings",
        ),
        (
            '[compat]\ndrop_params = [" "]\n',
            ValueError,
...
def test_drop_params_rejects_malformed_config(
    ...
    with pytest.raises(error_type, match=message):
        config.drop_params_from_config(config.load_config(config_path))
